"""
ASC File Reader and Decoder

This module provides a high-level interface for reading and decoding ASC (ASCII log)
files using DBC (CAN Database) files.

Example:
    >>> from asc_python import ASC
    >>> asc = ASC('recording.asc', [(-1, 'example/IMU.dbc')])
    >>> # Access signal data
...
from . import asc_python as _asc_c


class MessageProxy:
    """Proxy object for dictionary-style access to signals within a message."""

    def __init__(self, asc: _asc_c.ASC, message_name: str) -> None:
    ...
    def get_signal_names(self) -> list[str]:
        ...
        return self._asc.get_signals(self._message_name)
    ...
    def get_signal(self, signal_name: str) -> NDArray[np.float64]:
        ...
        return self._asc.get_signal(self._message_name, signal_name)
    ...
    def get_all_signals(self) -> NDArray[np.float64]:
        ...
        if self._cached_data is None:
            self._cached_data = self._asc.get_message_data(self._message_name)
    ...
    def get_signal_units(self) -> dict[str, str]:
        ...
        if self._cached_units is None:
            self._cached_units = self._asc.get_signal_units(self._message_name)
    ...
    def get_signal_factors(self) -> dict[str, float]:
        ...
        if self._cached_factors is None:
            self._cached_factors = self._asc.get_signal_factors(self._message_name)
    ...
    def get_signal_offsets(self) -> dict[str, float]:
        ...
        if self._cached_offsets is None:
            self._cached_offsets = self._asc.get_signal_offsets(self._message_name)
    ...
    def get_period(self) -> int:
        ...
        Example:
            >>> period = asc['GpsStatus'].get_period()
            >>> print(f"Sampling period: {period} ms")
        """
        return self._asc.get_period(self._message_name)
    ...
    def __contains__(self, signal_name: str) -> bool:
        ...
        try:
            signals = self._asc.get_signals(self._message_name)
...
class ASC:
    """
    ASC file reader with DBC decoding support.

    This class reads ASC (ASCII log) files and decodes CAN messages using provided DBC database files.
    ...
    Example:
        ...
        >>> print(asc.messages)  # List all decoded messages
        >>> print(asc.get_signals('GpsStatus'))  # List signals in a message
        >>> # Access signals
        >>> timestamps = asc.get_signal('GpsStatus', 'Time')
        >>> gps_mode = asc.get_signal('GpsStatus', 'GpsPosMode')
        >>> # Or use dictionary-style access
        ...
        >>> factor = asc.get_signal_factor('Distance', 'Distance')
    """

    # Class-level cache for instances
    _instances: dict[tuple[Path, tuple[tuple[int, str], ...]], "ASC"] = {}

    def __new__(cls, filepath: str | Path, channel_dbc_list: list[tuple[int, str | Path]]) -> "ASC":
        """
        Create or return cached ASC instance.

        Args:
            filepath: Path to the ASC file (str or Path)
            channel_dbc_list: List of (channel, dbc_filepath) tuples

        Returns:
            ASC instance (cached if filepath and channel_dbc_list were previously used)
        """
    ...
    def __init__(self, filepath: str | Path, channel_dbc_list: list[tuple[int, str | Path]]) -> None:
        """
        Initialize ASC reader and decode the file.
        ...
        Args:
            filepath: Path to the ASC file to read (str or Path)
            channel_dbc_list: List of (channel, dbc_filepath) tuples
                Each tuple specifies which DBC file to use for which CAN channel.
                Use -1 for wildcard (matches all channels)

        Raises:
            FileNotFoundError: If ASC file or any DBC file doesn't exist
            ValueError: If channel_dbc_list is empty or contains invalid entries
            IOError: If ASC file cannot be opened or read

        Example:
            >>> # Single DBC for all channels (wildcard)
            >>> asc = ASC('recording.asc', [(-1, 'vehicle.dbc')])
            >>> # Specific channel
            >>> asc = ASC('recording.asc', [(4, 'vehicle.dbc')])
            >>> # Multiple channels with different DBCs
            >>> asc = ASC('recording.asc', [(1, 'powertrain.dbc'), (2, 'chassis.dbc')])

        Note:
            The ASC file is processed immediately upon instantiation.
        ...
        if not filepath_obj.exists():
            raise FileNotFoundError(f"ASC file not found: {filepath}")
    ...
    def get_period(self, message_name: str) -> int:
        ...
        Example:
            >>> period = asc.get_period('GpsStatus')
            >>> print(f"Sampling period: {period} ms")
        """
        return self._asc.get_period(message_name)
    ...
    def __getitem__(self, message_name: str) -> MessageProxy:
        ...
        if message_name not in self._message_proxies:
            self._message_proxies[message_name] = MessageProxy(self._asc, message_name)
        return self._message_proxies[message_name]

    def __contains__(self, message_name: str) -> bool:
        """Check if a message exists in the ASC file."""
    ...
    def __repr__(self) -> str:
        """String representation of ASC object."""
        channels = [ch for ch, _ in self.channel_dbc_list]
        return f"ASC(filepath='{self.filepath}', messages={len(self.get_message_names())}, channels={channels})"

    def __str__(self) -> str:
        """Human-readable string representation."""
        lines = [f"ASC File: {self.filepath}"]
    ...
    def info(self) -> None:
        """Print detailed information about the loaded ASC file."""
        print(str(self))
