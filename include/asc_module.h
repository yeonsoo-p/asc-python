/*----------------------------------------------------------------------------
| ASC Python Module Header - Common definitions, macros, and structures
|-----------------------------------------------------------------------------
| This header provides common definitions for the ASC Python C extension module
...
#define ASC_FASTCALL(func) static PyObject* func(ASCObject* self, PyObject* const* args, Py_ssize_t nargs)
#define ASC_NOARGS(func)   static PyObject* func(ASCObject* self, PyObject* Py_UNUSED(args))

// Validation macros
#define ASC_CHECK_PARSED                                                \
    do {                                                                \
        if (!self->parsed) {                                            \
            PyErr_SetString(PyExc_RuntimeError, "ASC file not loaded"); \
            return NULL;                                                \
        }                                                               \
    } while (0)

#define ASC_CHECK_NARGS(expected)                                                                                                                     \
    do {                                                                                                                                              \
        if (nargs != (expected)) {                                                                                                                    \
...
#define ASC_GET_STRING_ARG(var, index)                                       \
    do {                                                                     \
...
#define ASC_FIND_MESSAGE(msg_var, msg_name)                               \
...
#define ASC_FIND_SIGNAL(msg_var, sig_var, sig_name)                      \
